        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px', scrollbarWidth: 'thin', scrollbarColor: '#e2e8f0 transparent' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
            {stats.map(s => {
              const Icon = s.icon;
              return (
                <div key={s.label} style={{ ...card, padding: '22px', transition: 'all 0.2s' } as React.CSSProperties}
                ...
                >
                  ...
                  <div style={{ fontSize: 30, fontWeight: 600, color: '#0f172a', letterSpacing: '-0.03em', marginBottom: 4, textAlign: 'right' }}>{s.value.toLocaleString()}</div>
                  <div style={{ fontSize: 12.5, fontWeight: 500, color: '#94a3b8' }}>{s.label}</div>
                </div>
              );
            })}
          </div>
