...
export default function PasswordPolicyPage() {
  ...
  // ✅ Always reads fresh token at call time — fixes 403
  const getHeaders = () => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('nexum_token') : null;
    return {
  ...
  const fetchPolicy = useCallback(async () => {
    ...
    try {
      ...
      setPolicy({
        ...
        requireUppercase: data.requireUppercase ?? data.RequireUppercase,
        requireLowercase: data.requireLowercase ?? data.RequireLowercase,
        requireNumbers:   data.requireNumbers   ?? data.RequireNumbers,
        requireSpecial:   data.requireSpecial   ?? data.RequireSpecial,
        blockCommon:      data.blockCommon       ?? true,
        expiryDays:       data.expiryDays       ?? data.ExpiryDays,
        historyCount:     data.historyCount     ?? data.HistoryCount,
      });
    } catch (e) {
  ...
  const fetchHistory = useCallback(async (page = 1) => {
    try {
      ...
      const data = await res.json();
      // normalise camelCase vs PascalCase from backend
  ...
  useEffect(() => { if (activeTab === 'history') fetchHistory(histPage); }, [activeTab, histPage]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        show('Access denied — Admin role required to save policy', 'error');
        return;
      }
      if (!res.ok) throw new Error();
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#ffffff', fontFamily: 'DM Sans, sans-serif' }}>
        ...
        <div style={{ flex: 1, padding: '28px 32px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{
                ...
                background: 'linear-gradient(135deg, #2db9a3 0%, #22a090 100%)',
          ...
          {activeTab === 'policy' && (
            <div style={{
              ...
              background: '#f0fdf9', border: '1.5px solid #a7f3e8',
            ...
            loading ? (
              ...
              <>
                <SectionCard title="Password Requirements" icon={<Shield size={16} />}>
                  <PolicyRow label="Minimum Length" description="Enforced minimum is 12 characters">
                    <StyledSelect
                      ...
                      onChange={v => update('minLength', Math.max(12, parseInt(v)))}
                  ...
                  <PolicyRow label="Require Uppercase Letters" description="At least one uppercase letter (A–Z)">
                    <Toggle checked={policy.requireUppercase} onChange={v => update('requireUppercase', v)} />
                  </PolicyRow>
                  <PolicyRow label="Require Lowercase Letters" description="At least one lowercase letter (a–z)">
                    <Toggle checked={policy.requireLowercase} onChange={v => update('requireLowercase', v)} />
                  </PolicyRow>
                  <PolicyRow label="Require Numbers" description="At least one numeric digit (0–9)">
                    <Toggle checked={policy.requireNumbers} onChange={v => update('requireNumbers', v)} />
                  </PolicyRow>
                  <PolicyRow label="Require Special Characters" description="At least one special character (!@#$%^&*)">
                    <Toggle checked={policy.requireSpecial} onChange={v => update('requireSpecial', v)} />
                  </PolicyRow>
                  <PolicyRow label="Block Common Passwords" description="Reject commonly used or weak passwords" last>
                    <Toggle checked={policy.blockCommon} onChange={v => update('blockCommon', v)} />
                  </PolicyRow>
                </SectionCard>
                ...
                <SectionCard title="Expiry & History" icon={<RotateCw size={16} />}>
                  <PolicyRow label="Password Expiry" description="Force password change after this many days (0 = never)">
                    <StyledSelect
                      ...
                      onChange={v => update('expiryDays', parseInt(v))}
                      options={[
                        { label: 'Never', value: 0 },
                  ...
                  <PolicyRow label="Password History" description="Prevent reuse of last N passwords" last>
                    <StyledSelect
                      ...
                      onChange={v => update('historyCount', parseInt(v))}
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8, paddingBottom: 8 }}>
                  <button onClick={fetchPolicy} style={{
                  ...
                  <button onClick={handleSave} disabled={saving} style={{
                    ...
                    background: saving ? '#94a3b8' : '#2db9a3',
                    color: '#fff', border: 'none', borderRadius: 9,
                    padding: '10px 24px', fontSize: 13, fontWeight: 600,
                    fontFamily: 'DM Sans, sans-serif',
                    cursor: saving ? 'not-allowed' : 'pointer',
                    boxShadow: '0 2px 8px rgba(45,185,163,0.25)',
                  }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                    ...
                    <tbody>
                      {history.map((row) => (
                        <tr
                          key={row.id}
                          style={{ borderBottom: '1px solid #f8fafc', transition: 'background 0.13s' }}
                        ...
                        >
                          <td style={{ padding: '14px 20px', textAlign: 'center', color: '#64748b', fontSize: 13, fontWeight: 500, verticalAlign: 'middle' }}>
                            {new Date(row.changedAt).toLocaleString('en-US', {
                          ...
                          <td style={{ padding: '14px 20px', textAlign: 'center', color: '#0f172a', fontWeight: 700, fontSize: 13.5, verticalAlign: 'middle' }}>
                            {row.employeeName}
                          </td>
                          <td style={{ padding: '14px 20px', textAlign: 'center', color: '#64748b', fontSize: 13, fontWeight: 500, verticalAlign: 'middle' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px', borderTop: '1px solid #f1f5f9', background: '#fafbfc' }}>
                    <span style={{ fontSize: 13, color: '#94a3b8' }}>
                      ...
                      <strong style={{ color: '#475569', fontWeight: 600 }}>
                        {histTotal === 0 ? 0 : (safePage - 1) * HIST_PER_PAGE + 1}–{Math.min(safePage * HIST_PER_PAGE, histTotal)}
                      </strong>
                      {' '}of{' '}
                      <strong style={{ color: '#475569', fontWeight: 600 }}>{histTotal}</strong>
                      {' '}records
                    </span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <button
                        ...
                        style={{
                          ...
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          color: '#64748b', opacity: safePage === 1 ? 0.35 : 1,
                          transition: 'all 0.15s',
                        }}
                      >
                        <ChevronLeft size={15} />
                      </button>
                      <span style={{ minWidth: 50, textAlign: 'center', fontSize: 13, color: '#475569', fontWeight: 500 }}>
                        {safePage} / {totalHistPages}
                      </span>
                      <button
                        onClick={() => setHistPage(p => Math.min(totalHistPages, p + 1))}
                        disabled={safePage === totalHistPages}
                        style={{
                          width: 34, height: 34, borderRadius: 8,
                          border: '1.5px solid #e2e8f0', background: '#fff',
                          cursor: safePage === totalHistPages ? 'not-allowed' : 'pointer',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          color: '#64748b', opacity: safePage === totalHistPages ? 0.35 : 1,
                          transition: 'all 0.15s',
                        }}
                      >
      ...
      {toast && createPortal(
        <div style={{
          ...
          background: toast.type === 'error' ? '#fef2f2' : '#f0fdf9',
          border: `1.5px solid ${toast.type === 'error' ? '#fca5a5' : '#a7f3e8'}`,
          color: toast.type === 'error' ? '#dc2626' : '#0f766e',
