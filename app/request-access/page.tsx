...
export default function RequestAccessPage() {
  ...
  useEffect(() => {
    if (!formData.password || formData.password.length < 8) { setPwnedWarning(false); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (pwErrors.length > 0)         return setError('Password requirements not met:\n' + pwErrors.join('\n'));
    if (pwnedWarning)                return setError('This password was found in a known data breach. Please choose a different password.');
    if (formData.password !== formData.confirmPassword) return setError('Passwords do not match.');
    if (!formData.agreeToTerms)      return setError('Please agree to the terms and access policy.');
    ...
    try {
      ...
      if (!valData.valid) return setError('Password requirements not met:\n' + (valData.errors ?? []).join('\n'));
    } catch {}
    ...
    try {
      ...
      if (!ok || !data.success) {
        if (data.message?.toLowerCase().includes('email'))
          return setError('This email address is already registered. Please use a different email.');
        if (data.message?.toLowerCase().includes('employee'))
          return setError('This Employee ID is already in use or has a pending request.');
        return setError(data.message || 'Submission failed. Please try again.');
      }
      ...
      <div className="w-full lg:w-1/2 lg:ml-auto flex items-center justify-center p-4 lg:p-8 overflow-y-auto">
        ...
        >
          ...
          <div className="text-center space-y-2 mb-6">
            <h2 className="font-bold leading-tight" style={{ fontSize: 'var(--font-size-2xl)', color: 'white' }}>
              Request Access
            </h2>
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'hsl(210,15%,55%)' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                <div key={name} className="space-y-2">
                  <label className="font-semibold block" style={{ fontSize: 'var(--font-size-sm)', color: 'hsl(210,15%,70%)' }}>{label}</label>
                  <div className="rounded-xl transition-all duration-300" style={{ boxShadow: focused === name ? focusGlow : 'none' }}>
                    <input
                      ...
                      style={{ ...glassInput, height: 'var(--mobile-input-height)', fontSize: 'var(--font-size-sm)' }}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-2">
              <label className="font-semibold block" style={{ fontSize: 'var(--font-size-sm)', color: 'hsl(210,15%,70%)' }}>Employee ID</label>
              <div className="rounded-xl transition-all duration-300" style={{ boxShadow: focused === 'employeeId' ? focusGlow : 'none' }}>
                <input placeholder="EMP001" name="employeeId" value={formData.employeeId}
                  onChange={handleInputChange} onFocus={() => setFocused('employeeId')} onBlur={() => setFocused(null)}
                  className="w-full px-4 rounded-xl outline-none transition-colors placeholder:text-[hsl(210,10%,35%)]"
                  style={{ ...glassInput, height: 'var(--mobile-input-height)', fontSize: 'var(--font-size-sm)' }} />
              </div>
            </div>
            ...
            {/* Email */}
            <div className="space-y-2">
              <label className="font-semibold block" style={{ fontSize: 'var(--font-size-sm)', color: 'hsl(210,15%,70%)' }}>Email Address</label>
              <div className="rounded-xl transition-all duration-300" style={{ boxShadow: focused === 'email' ? focusGlow : 'none' }}>
                <input type="email" placeholder="john@bank.com" name="email" value={formData.email}
                  onChange={handleInputChange} onFocus={() => setFocused('email')} onBlur={() => setFocused(null)}
                  className="w-full px-4 rounded-xl outline-none transition-colors placeholder:text-[hsl(210,10%,35%)]"
                  style={{ ...glassInput, height: 'var(--mobile-input-height)', fontSize: 'var(--font-size-sm)' }} />
              </div>
            </div>
            ...
            {/* ✅ Phone Number */}
            <div className="space-y-2">
              <label className="font-semibold block" style={{ fontSize: 'var(--font-size-sm)', color: 'hsl(210,15%,70%)' }}>
                Phone Number <span style={{ fontSize: 10, fontWeight: 700, color: 'hsl(170,60%,50%)', marginLeft: 4 }}>FOR SMS OTP</span>
              </label>
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', padding: '0 12px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'hsl(210,15%,70%)', fontSize: 14, flexShrink: 0, height: 'var(--mobile-input-height)' }}>
                  +63
                </div>
                <div className="flex-1 rounded-xl transition-all duration-300" style={{ boxShadow: focused === 'phone' ? focusGlow : 'none' }}>
                  <input
                    type="tel" name="phone" placeholder="9XXXXXXXXX"
              ...
              <p style={{ fontSize: 11, color: 'hsl(210,15%,45%)', marginTop: 2 }}>
                Optional. Used to receive SMS OTP during login if MFA is enabled.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative rounded-xl transition-all duration-300" style={{ boxShadow: focused === 'password' ? focusGlow : 'none' }}>
                <input
            ...
            <div className="space-y-2">
              ...
              <div className="relative rounded-xl transition-all duration-300" style={{ boxShadow: focused === 'confirmPassword' ? focusGlow : 'none' }}>
                <input
            ...
            {error && (
              <div className="rounded-xl px-4 py-3 space-y-1" style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', color: 'rgb(252,165,165)', fontSize: 'var(--font-size-sm)' }}>
                {error.split('\n').map((line, i) => <p key={i}>{i === 0 ? `⚠ ${line}` : `  • ${line}`}</p>)}
              </div>
            )}
